...
import { TrendingUp, TrendingDown, Landmark, Bitcoin, Briefcase, ArrowRightLeft, Search } from 'lucide-react';
...
const SymbolIcon = ({ type }) => {
  ...
  if (type === 'forex')  return <ArrowRightLeft  className="h-5 w-5 mr-2 text-purple-400" />;
  return null;
};

const MarketSymbols = () => {
  const { symbols, setSelectedSymbol, selectedSymbol, livePrices } = useTradingContext();
      ...
      <div className="space-y-2 max-h-[400px] overflow-y-auto">
        {filteredSymbols.length === 0 ? (
          ...
          filteredSymbols.map((symbol) => {
            // Use live price from scheduler if available, fallback to historical
            const live        = livePrices?.[symbol.id];
            const livePrice   = live?.price   ?? symbol.price;
            const liveChange  = live?.percentChange ?? symbol.change;
            const isLive      = !!live && live.source === 'live';
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <p className="font-medium">{formatCurrency(livePrice, symbol.currency)}</p>
                    <p className={`text-sm flex items-center justify-end ${
                      liveChange >= 0 ? 'price-up' : 'price-down'
                    }`}>
                      {liveChange >= 0
                        ? <TrendingUp  className="h-3 w-3 mr-1" />
                        : <TrendingDown className="h-3 w-3 mr-1" />
                      }
                      {formatPercentage(liveChange)}
                    </p>
                  </div>
                </div>
              </motion.div>
